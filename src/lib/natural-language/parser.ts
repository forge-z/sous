...
function parseAdd(text: string): ParsedAction[] {
  return text.replace(/^(comprei|adicion(ei|ar)?|bought|added)\\s+/i, "").split(/\\s+e\\s+|\\s*,\\s*/i).map((part) => {
    ...
    return { action: "add" as const, item: clean(match[3]), quantity, unit: aliases[(match[2] ?? "unit").toLocaleLowerCase()] ?? "unit" };
  }).filter((item): item is ParsedAction => Boolean(item));
}

export function parseInventoryCommand(text: string): ParsedAction[] {
  const value = clean(text);
  if (!value) return [];
  if (/(acabou|finished|ran out|is out of)/i.test(value)) {
    const item = clean(value.replace(/^(acabou|finished|ran out of|is out of)\\s*/i, "").replace(/^(o|a|the|do|da|de)\\s+/i, ""));
  ...
  if (/^(usei|consumi|used|ate|comi)\\b/i.test(value)) {
    const match = value.match(/^(?:usei|consumi|used|ate|comi)\\s+(?:(metade|half)\\s+(?:de|of)\\s+)?(?:o|a|do|da|the)?\\s*(.+)$/i);
    return match ? [{ action: "consume", item: clean(match[2]), quantity: match[1] ? null : 1 }] : [];
  }
